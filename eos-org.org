...
#+BEGIN_SRC emacs-lisp
  (use-package org
    ...
    (progn
      ...
      (setq ;; org-directory (file-truename "~/org")
            ...
            org-agenda-files (quote ("~/org" "~/org/intrasee")))
      ...
      (setq org-todo-keyword-faces
            '(("TODO" :foreground "red" :weight bold)
             ...
             ("YES" :foreground "forest green" :weight bold)
             ("NO" :foreground "red" :weight bold)
             ("GEEVE" :foreground "orange" :weight bold)))
  ...
  ;;Moving a CANCELLED task back to TODO removes the CANCELLED tag. The tags are
  ;;used to filter tasks in the agenda views conveniently.
...
(global-set-key (kbd "C-<f11>") #'eos/org-clock-in)
...
* BH setup
:PROPERTIES:
:CUSTOM_ID: h:97049d07-8d6d-4da8-81ee-51eb576d8758
:END:
...
(global-set-key (kbd "<f9> G") 'gnus-no-server)
...
;;let's see if this sets the point in the right place when adding a heading
...
;;no multiple timestamps for the same entry in the agenda views
(setq org-agenda-skip-additional-timestamps-same-entry t)

;;org-indent mode on startup
...
:PROPERTIES:
:CUSTOM_ID: h:6f45e579-8ffc-46d4-bf44-4dfee0349c75
:END:

#+begin_src emacs-lisp
(use-package org-sync
  :load-path "/home/cc/.emacs.d/org-sync/"
  :ensure t
  :config
  (mapc 'load
